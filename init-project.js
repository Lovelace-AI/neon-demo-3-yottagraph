...
function writeAetherDataModeFile(mode) {
    ...
    fs.writeFileSync(path.join(cursorDir, '.aether-data-mode'), mode + '\n', 'utf-8');
}
...
function extractAetherInstructions(dataMode = 'api-mcp') {
    ...
    try {
        ...
        // .cursor/rules/ directory left over from pre-skill installs.
        const legacyRulesDir = path.join(cursorDir, 'rules');
        if (fs.existsSync(legacyRulesDir)) {
            fs.rmSync(legacyRulesDir, { recursive: true, force: true });
        }
        const legacyCmdDir = path.join(cursorDir, 'commands');
        ...
        if (dataMode === 'mcp-only') {
            ...
            const elementalSkill = path.join(cursorDir, 'skills', 'elemental-api');
...
Options:
  ...
  --data-mode <mode>          api-mcp (default) or mcp-only — Cursor instructions variant
