...
- **`.cursor/skills/aether/`** -- the Aether skill: architecture, APIs, UI patterns, agents, MCP servers, deployment (read `SKILL.md` first)
- **`DESIGN.md`** -- project vision that agents read first
- **`design/*.md`** -- feature docs for collaborative planning
- **`.cursor/skills/`** -- additional skill docs (populated by `npm install` and `/update_instructions`)
